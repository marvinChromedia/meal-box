...
- `frontend/src/components/ui/` — fourteen UI components, including `EmptyState`, `LoadingState` and `ErrorState` for the states every data-backed screen needs (TEST-157), and a `size="lg"` variant on `Checkbox` for a one-handed checklist (TEST-77); see [`design-system.md`](./design-system.md).
...
- `frontend/src/features/shopping-list/{ShoppingList,ShoppingListItemRow}.tsx` — the shopping-list screen (TEST-77), routed at `/shopping-list`: combined-quantity line items, provenance (which recipes contributed, or "Recipe removed" for a since-deleted one), optimistic check-off, manual add/remove.

Both the recipes and shopping-list HTTP surfaces now exist (TEST-72, TEST-76), but **the frontend's shopping-list client and the real backend currently disagree on paths** — the client calls `POST /api/shopping-list/generate` and per-item endpoints (`/shopping-list/items`, `/shopping-list/items/:id`) that don't exist on the server yet; the server only implements `POST /api/shopping-list` (generate) and `GET /api/shopping-list`. Tracked as [TEST-234](https://beacon.chro.media/browse/TEST-234). Until that lands, `frontend/src/features/shopping-list/api.ts` runs on its typed mock (`VITE_API_MODE=mock`, the default) — recipes' `api.ts` already matches its real endpoints and can run in `http` mode today.
