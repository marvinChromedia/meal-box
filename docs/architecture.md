...
Both the recipes and shopping-list HTTP surfaces now exist (TEST-72, TEST-76, TEST-234 — generation, reading the list, and the per-item add/update/remove endpoints TEST-77 and TEST-154 need). The frontend's `api.ts` files still default to their typed mocks (`VITE_API_MODE=mock`) — a screen ticket flips to `http` once it's ready to consume the real endpoints, per AC5's mock/swap contract. The shopping-list client and the backend now agree on every path it calls (they briefly didn't — see below) — `frontend/src/features/shopping-list/api.ts` can move to `http` mode the same way recipes' already has.
